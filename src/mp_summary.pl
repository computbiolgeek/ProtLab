...
my $header = sprintf(
	...
	"pdbid",      "total",        "tmh_res",  "helices",
	"tm_helices", "bent_helices", "subunits", "family_id"
);
print $fh_out $header;
...
foreach my $pdbid (@pdbids) {
	...
	unless ( looks_like_number($half_thickness) ) {
		carp
"$pdb_file_opm does not seem to contain a valid half-thickness, please check";
		next;
	}
	...
	my $tmh_res =
	  get_number_tmh_residues( $pdbid, $half_thickness, \@helices,
		$pdb_hash_chain{ATOM} );

	# get the number of transmemrbane helices
	my $tm_helices =
	  get_number_tmh( $pdbid, $half_thickness, \@helices,
		$pdb_hash_chain{ATOM} );

	# get the number of bent helices
	my $bent_helices =
	  get_number_bent_helix( $pdbid, \@helices, $pdb_hash_chain{ATOM} );
	...
	my $out_line  = sprintf( "%-8s%8d%14d%14d%14d%14d%14d%14d\n",
		$pdbid, $total, $tmh_res, $helices, $tm_helices, $bent_helices,
		$subunits, $family_id );
...
sub is_tmh_pdb {
	...
	if (   $middle_z <= $half_thickness
		&& $middle_z >= -$half_thickness
		&& ( $end - $start ) >= 12 )
	{
		return 1;
	}
...
sub get_ca_coordinates {
	...
	unless (@residue_records) {
		croak
"Missing coordinates for residue $seqid in the pdb file, please check";
	}
...
sub distance {
	my ( $pointA, $pointB ) = @_;
	return
	  sqrt( sum( map { ( $pointA->[$_] - $pointB->[$_] )**2 } ( 0 .. 2 ) ) );
}
...
sub get_number_bent_helix {
	...
	foreach my $helix ( @{$helices} ) {
		...
		my $is_bent_helix =
		  ( $actual_length - $ideal_length ) / $ideal_length >= 0.1 ? 1 : 0;
		if ($is_bent_helix) {
			$count++;
		}
	}
...
sub get_family_id {
	...
	elsif ( $html =~ /protein.php\?pdbid=([0-9a-z]+)/ )
	{    # get representative pdb
