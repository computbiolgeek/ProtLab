...
def main():
    ...
    with open(args.res[0], 'rt') as f:
        for line in f.readlines():
            residues_a += re.split(r'[,;\s]\*', line.strip())
    residues_a = [int(i) for i in residues_a]
    residues_b = []
    with open(args.res[1], 'rt') as f:
        for line in f.readlines():
            residues_b += re.split(r'[,;\s]\s*', line.strip())
    residues_b = [int(i) for i in residues_b]
    ...
    mi_df = pd.DataFrame(np.array(mi), index=residues_a, columns=residues_b)
